...
let string_of_linear_expression = function
...
let string_of_boolean_expression (cmp : boolean_expression) =
    ...
    | True -> "true"
    | False -> "false"
    | BoolExpr (l, op, r) -> (string_of_linear_expression l) ^ (string_of_op op) ^ (string_of_linear_expression r)
    

let string_of_implication impl =
    if impl.assumption = True then (string_of_boolean_expression impl.conclusion) 
    else "(" ^ (string_of_boolean_expression impl.assumption) ^ " &rArr; " ^ (string_of_boolean_expression impl.conclusion) ^ ")"    

let string_of_formula = function
    | [] -> "true"
    | impl :: tail -> List.fold_left (fun a i -> a ^ " &and; " ^ (string_of_implication i)) (string_of_implication impl) tail 

let string_of_instruction = function
    | Assignment (v, e) -> v ^ " = " ^ (string_of_linear_expression e)
    | Read v -> v ^ " = read()"
    | Write v -> "write(" ^ v ^ ")"
...
let print_cfg (g : cfg) (fname : string) : unit =
    ...
    let print_cfg_edge (src, grd, ass, dst) =
        Printf.fprintf out "\tn%i -> n%i[taillabel=\"%s\",label=\"%s\"];\n" src dst
            (match grd with None -> "" | Some No -> "no" | Some Yes -> "yes") (match ass with None -> "" | Some a -> string_of_formula a)
    in
    begin
