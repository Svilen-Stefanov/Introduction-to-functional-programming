...
(*let rec insert_rec key data tree (k : 'k key_info) = 
  match tree with 
    Node n ->
      if (k.compare key (get_key n.data) > 0)
        then Node {n with right = insert_rec key data n.right k}
        else Node {n with left = insert_rec key data n.left k}
  | _ -> Node { data = (key, data); left = Leaf; right = Leaf }*)

  (*t.tree <- insert_rec key data (t.tree) (t.hash_fun);*)